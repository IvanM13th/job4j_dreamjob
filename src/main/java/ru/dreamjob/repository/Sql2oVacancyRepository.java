...
public class Sql2oVacancyRepository implements VacancyRepository {
    ...
    public Vacancy save(Vacancy vacancy) {
        try (var connection = sql2o.open()) {
            var sql = """
                    insert into vacancies (title, description, creation_time, visible, city_id, file_id)
                    values (:title, :description, :creationTime, :visible, :cityId, :fileId)
                    """;
            var query = connection.createQuery(sql, true)
                    .addParameter("title", vacancy.getTitle())
                    .addParameter("description", vacancy.getDescription())
                    .addParameter("creationTime", vacancy.getCreationTime())
    ...
    public boolean update(Vacancy vacancy) {
        try (var connection = sql2o.open()) {
            var sql = """
                    UPDATE vacancies
                    SET title = :title, description = :description, creation_time = :creationTime,
